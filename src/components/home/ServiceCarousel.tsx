        ...
        {services.map((service, idx) => (
          <div key={idx} className="w-[85vw] md:w-[calc(33.333%-16px)] shrink-0 snap-start flex flex-col group/item cursor-pointer">
            <Link href={`/services/${service.title.toLowerCase().replace(' ', '-')}`}>
              ...
              <h3 className="font-serif text-2xl font-normal text-text-main tracking-wide mb-2 group-hover/item:text-[#a85b9b] transition">
                {service.title}
              </h3>
              <p className="font-sans text-[15px] text-text-muted mb-4 leading-relaxed">
                {service.desc}
              </p>
              <span className="text-[14px] font-sans text-text-main border-b border-text-main pb-1 transition group-hover/item:text-[#a85b9b] group-hover/item:border-[#a85b9b]">
                Explore →
              </span>
            </Link>
          </div>
